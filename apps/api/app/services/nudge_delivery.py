...
def is_on_cooldown(user_id: int, new_trigger_rule: str, db: Session) -> bool: # crisis triggers skip the cooldown completely, since waiting a couple of days could mean a genuinely new crisis notification gets held back just because a milder one went out the day before, and missing a real crisis is worse than sending an extra notification. duplicate spam is already handled separately by the 20 hour check in create_nudge_flag, so nothing extra is needed here. everything else still waits the standard week since the last notification actually went out
